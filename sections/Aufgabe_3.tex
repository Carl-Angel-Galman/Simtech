...
Aus der Aufgabenstellung \eqref{eq:betadot}:
\begin{equation*}
...
Aus der Aufgabenstellung \eqref{eq:psiddot}:
\begin{equation*}
\ddot{\Psi} \;=\; \frac{1}{J}
\left(
...
Aus der Aufgabenstellung \eqref{eq:alphav}:
\begin{equation*}
...
Aus der Aufgabenstellung \eqref{eq:alphah}:
\begin{equation*}
...
Für die Sprünge $\delta = 5^\circ$ und $\delta = 20^\circ$ wurden beide Modelle nacheinander simuliert.
Zusatzlich wurde die Differenz zwischen linearem und nichtlinearem Modell als Verlauf im Scope dargestellt und der betragmässig maximale Wert im Modell berechnet und auf einem Display ausgegeben.

\subsubsection{Sprung auf $\delta = 5^\circ$}
Die folgenden Abbildungen zeigen den Vergleich der linearen und nichtlinearen Simulation sowie die zugehörigen Differenzsignale für den Sprung auf $\delta = 5^\circ$.
...
Abbildung~\ref{fig:aufgabe3sprung5} zeigt, dass das linearisierte System für den kleinen Sprung $\delta = 5^\circ$ die nichtlineare Dynamik nahezu deckungsgleich abbildet.
Die Differenzsignale bleiben sehr klein, und die maximale Abweichung ist entsprechend gering.
Auf dem Display wurden die betragmässig maximalen Differenzen mit 0.0031 ($\beta$ in Grad) und 0.0569 ($\dot{\Psi}$ in Grad/s) ausgegeben.

\subsubsection{Sprung auf $\delta = 20^\circ$}
Die folgenden Abbildungen zeigen den Vergleich der linearen und nichtlinearen Simulation sowie die zugehörigen Differenzsignale für den Sprung auf $\delta = 20^\circ$.
...
Auf dem Display wurden die betragmässig maximalen Differenzen mit 0.1947 ($\beta$ in Grad) und 3.6382 ($\dot{\Psi}$ in Grad/s) ausgegeben.
Dies liegt daran, dass der Sprung weit vom Linearisierungsarbeitspunkt entfernt ist und die Vernachlässigung der nichtlinearen Terme nicht mehr gerechtfertigt ist.